				...
				<Sheet open={open} onOpenChange={() => setOpen(value => !value)}>
					...
					<SheetContent side={"left"} className="bg-bgDefault text-white rounded-br-3xl rounded-tr-3xl text-xs bg-opacity-75 ">
						...
						<div className="flex flex-col gap-4 py-8">
							<Link href="/" className="uppercase cursor-pointer" onClick={() => setOpen(false)}>
								anasayfa
							</Link>
							...
							<Link href="/corporate" className="uppercase" onClick={() => setOpen(false)}>
								kurumsal
							</Link>
							<Link href="/activities" className="uppercase" onClick={() => setOpen(false)}>
								faaliyetler
							</Link>
							<Link href="/aid" className="uppercase" onClick={() => setOpen(false)}>
								yardımlar
							</Link>
							<Link href="/contact" className="uppercase" onClick={() => setOpen(false)}>
								iletişim
							</Link>
							<Accordion type="multiple">
								<AccordionItem value="item-1">
									...
									<AccordionContent className="flex flex-col gap-4 text-xs">
										<span className="flex gap-2 hover:underline">
											<ChevronRight className="w-4 h-4" />
											<Link href="/kurumsal" className="uppercase cursor-pointer" onClick={() => setOpen(false)}>
												merkez sodam
											</Link>
										</span>
										<span className="flex gap-2 hover:underline">
											<ChevronRight className="w-4 h-4" />
											<Link href="/sehit-mustafa-tezgider-sodam" className="uppercase cursor-pointer" onClick={() => setOpen(false)}>
												şehit mustafa tezgider sodam
											</Link>
										</span>
										<span className="flex gap-2 hover:underline">
											<ChevronRight className="w-4 h-4" />
											<Link href="/sehit-ozer-camci-sodam" className="uppercase cursor-pointer" onClick={() => setOpen(false)}>
												şehit özer camcı sodam
											</Link>
										</span>
									</AccordionContent>
