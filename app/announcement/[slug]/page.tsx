    ...
    <div className="flex flex-col md:flex-row w-[96%] flex-wrap mx-auto gap-2 md:gap-4 text-xs sm:text-sm md:text-base lg:text-lg px-2">
      <div className="flex flex-col p-4 my-8 mx-auto shadow-xl shadow-red-200 rounded-lg basis-[27em] lg:basis-[36em] flex-grow flex-shrink">
        <h1 className="uppercase font-extrabold py-4 text-bgDefault">
          {announce?.program || "Program Not Available"}
        </h1>
        <ul>
          <li className="py-2 font-extrabold uppercase text-bgDefault">
            {announce?.title}
          </li>
  ...
  {announce?.content &&
    announce.content.map((item:{ children?: { text?: string }[] }) => item?.children?.[0]?.text || "No content available")}
      ...
      <div className="flex flex-wrap flex-row md:flex-col mx-auto flex-grow flex-shrink">
        <h1 className="text-center m-4 p-4 rounded-lg text-white bg-bgDefault uppercase font-extrabold">
          Duyurular
        </h1>
        <div className="flex flex-wrap flex-row md:flex-col mx-auto flex-grow flex-shrink">
